...
def loop_of_match(adjmat, redundancy_mask, redundancy_node):
    ...
    torch.save(redun_mat,       '../data/split_adj_matrix/%s_r_mat.pt'%name)
    torch.save(redun_node.T,    '../data/split_adj_matrix/%s_qit.pt'%name)
    torch.save(remain,          '../data/split_adj_matrix/%s_u_mat.pt'%name)
    #return redun_mat, redun_node, boost


def nvn():
    adj_matrix = torch.load("../data/original_adj_matrix/%s_matrix.pt" % name).float().to(device)
    red_mask = torch.load('../search/output/%s_redun.pt' % name).to(device)
